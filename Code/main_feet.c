...
ISR(TIMER0_COMPA_vect) {
    g_timer_ms_count++;
    if (g_timer_ms_count >= 20) {   // 20ms interval
        g_timer_ms_count = 0;
...
void
filter_accel_z(float current_z_g) {
...
main(void) {
    ...
    while (1) {
        if (g_read_sensor_flag) {
            ...
            // --- FEET THRESHOLD LOGIC ---
            // Range: 0 to 1
            // N = Neutral (0 to 0.33)
            // z = Slow forward (0.33 to 0.66)
            // Z = Fast forward (0.66 to 1.0)

            if (accel_z_filter < 0.22 && accel_z_filter > -0.22) {
                // Neutral - no acceleration
                uart_send('N', NULL);
            } 
            else if (accel_z_filter >= 0.66) {
                // High tilt (>= 0.66g) -> Fast forward
                uart_send('W', NULL);
            } 
            else if (accel_z_filter >= 0.22) {
                // Medium tilt (0.33g to 0.66g) -> Slow forward
                uart_send('w', NULL);
            }
            else if (accel_z_filter <= -0.44) {
                // High tilt break
                uart_send('S', NULL);
            }
            else if (accel_z_filter <= -0.22) {
                // Medium tilt break(0.33g to 0.66g) -> Slow break
                uart_send('s', NULL);
            }
