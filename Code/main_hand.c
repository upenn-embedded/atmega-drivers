...
float z_raw_filter;
float accel_z_filter = 0;
int z_filter_state = 0;

// --- Timer Interrupt Service Routine ---
// This ISR will be called every 1ms
ISR(TIMER0_COMPA_vect) {
    g_timer_ms_count++;
//    if (g_timer_ms_count >= 1000) { // 1000ms = 1 second
    if (g_timer_ms_count >= 20) { // 200ms = 1 second
        g_timer_ms_count = 0;
        g_read_sensor_flag = 1; // Set the flag for the main loop
    }
}

void filter_accel_z(float current_z_g) { 
    // Exponential Moving Average (Low Pass Filter)
    // This takes 90% of the old value and 10% of the new value
    accel_z_filter = (FILTER_ALPHA * current_z_g) + ((1.0 - FILTER_ALPHA) * accel_z_filter);
}
...
int main(void) {
    ...
    while (1) {
        if (g_read_sensor_flag) {
            ...
            // Filter the raw data using the new function
            filter_accel_z(accel_z_g);

            // --- MULTI-LEVEL THRESHOLD LOGIC ---
            // Values assume Z is positive for Left and negative for Right
            // Adjust signs based on your specific mounting orientation
            
            // DEADZONE: If between -0.2 and 0.2, go straight
            if (accel_z_filter > -0.2 && accel_z_filter < 0.2) {
                uart_send('N', NULL);
            }
            // LEFT TURNS (Positive Z)
            else if (accel_z_filter >= 0.7) {
                // High tilt (> 0.6g) -> Fast Left
                uart_send('A', NULL); 
            }
            else if (accel_z_filter >= 0.2) {
                // Medium tilt (0.2g to 0.6g) -> Slow Left
                // We send a lowercase 'a' to indicate a gentle turn
                uart_send('a', NULL); 
            }
            // RIGHT TURNS (Negative Z)
            else if (accel_z_filter <= -0.7) {
                // High tilt (<-0.6g) -> Fast Right
                uart_send('D', NULL);
            }
            else if (accel_z_filter <= -0.2) {
                // Medium tilt (-0.2g to -0.6g) -> Slow Right
                // We send a lowercase 'd' to indicate a gentle turn
                uart_send('d', NULL);
            }
        }
    }
